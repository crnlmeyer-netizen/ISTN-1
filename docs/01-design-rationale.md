...
## 1. Field Bus Topology Decisions

### 1.1 Multi-Drop Bus Consolidation vs. Dedicated Ports
* **Decision:** Solitary single-channel ports selected for SDI-12 (1x 3-pin) and RS-485 Modbus RTU (1x 4-pin).
...
## 2. Wireless Radio & Antenna Coexistence Architecture

### 2.1 Dual-IC Wireless Strategy (SIM7080G + Semtech SX1262)
...
### 3.1 Chemistry & Solar Input
* **Decision:** Single-cell LiFePO4 chemistry ($3.2\text{V}$ nominal, $3.6\text{V}$ float) paired with a dedicated MPPT buck charger IC and $6\text{V}\text{--}24\text{V}$ solar input stage.
